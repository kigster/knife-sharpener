  ...
  module Sharpener
    class CLI
      ...
      def run args = ARGV
        ...
        command_class = Sharpener::CLI.command(command_name)
        Sharpener::Exit.with_message("Command '#{command_name}' not found.") if command_class.nil?
      ...
      def validate!
        Sharpener::Exit.with_message('No command given') if args.empty?
        @command = args.first
      end

      def generate_config argv = []
        parse_options argv
        config[:configuration] = Sharpener::Configuration.from_path(config[:config_file])
      end

      def enable_logger
        STDOUT.sync = true
        Sharpener::Logger.enable
      end
...
require 'knife/sharpener/cli/provision'
require 'knife/sharpener/cli/bootstrap'
