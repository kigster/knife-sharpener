...
describe Sharpener::Command::Provision do
  let(:configuration) { Sharpener::Configuration.from_path('spec/fixtures/ec2.yml') }

  describe '#shell_commands' do
    context 'host is specified' do
      let(:subject) { Sharpener::Command::Provision.new(template_configuration, number: '1') }
    ...
    context 'host number is not specified' do
      let(:subject) { Sharpener::Command::Provision.new(template_configuration) }
      let(:template_configuration) { configuration.for_template('memcached-sessions') }
